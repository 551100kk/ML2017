...
inter = {}
with open('inter.pkl', 'rb') as fp:
   inter = pickle.load(fp)

print ('inter: %d words' % (len(inter)))
...
def text_proc(text):
    ...
    context = [i for i in context if inter.get(i) != None]
    return ((' ').join(context))
...
def main():
    ...
    with open('inter_tokenizer.pkl', 'wb') as fp:
       pickle.dump(tokenizer, fp, pickle.HIGHEST_PROTOCOL)'''


    with open('inter_tokenizer.pkl', 'rb') as fp:
